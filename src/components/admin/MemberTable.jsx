    ...
    <div className="flex-1 w-full space-y-8 overflow-hidden">
      ...
      <div className="bg-white/60 dark:bg-dark-card/40 backdrop-blur-3xl rounded-3xl shadow-premium dark:shadow-premium-dark border border-white/60 dark:border-white/10 overflow-hidden transition-all duration-500 hover:shadow-premium-hover dark:hover:shadow-premium-dark-hover">
        ...
        {loading ? (
          ...
          <>
            {/* Mobile / Compact Screen Card View */}
            <div className="md:hidden divide-y divide-gray-100 dark:divide-white/5">
              {currentMembersList.map((member) => (
                <div key={member.id} className="p-4 hover:bg-white/40 dark:hover:bg-white/5 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-100 dark:bg-dark-surface border border-gray-200/50 dark:border-white/10 flex items-center justify-center shrink-0">
                        {member.profileImage ? (
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="font-bold text-gray-900 dark:text-white font-sans text-sm truncate">{member.name}</h4>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate" title={member.email}>{member.email}</p>
                      </div>
                    </div>
                    ...
                    <div className="relative shrink-0">
                      ...
                      {activeDropdownId === member.id && (
                        <div ref={dropdownRef} role="menu" aria-label={`Actions for ${member.name}`} className="absolute right-0 top-8 w-36 bg-white dark:bg-dark-surface rounded-xl shadow-premium dark:shadow-premium-dark border border-gray-100 dark:border-white/10 overflow-hidden z-50 py-1">
                  ...
                  <div className="mt-2.5 flex items-center gap-2 flex-wrap pl-13">
                    <span className="px-2.5 py-0.5 bg-white/60 dark:bg-dark-surface border border-gray-200/50 dark:border-white/10 rounded-md text-[11px] font-bold text-indigo-700 dark:text-indigo-300 shadow-sm">
                      {member.role}
                    </span>
                    <span className="text-xs font-medium text-gray-600 dark:text-gray-300">
                      {getOrdinalYear(member.year)} Year
                    </span>
                  </div>
                </div>
              ))}
                    ...
                    <tr key={member.id} className="hover:bg-white/40 dark:hover:bg-white/5 transition-colors group">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-gray-900 dark:text-white font-sans text-sm whitespace-nowrap">{member.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400 text-[13px] font-medium max-w-[220px] truncate" title={member.email}>
                        {member.email}
                      </td>
                      ...
                      <td className="px-4 py-3 text-sm font-medium text-gray-600 dark:text-gray-300 whitespace-nowrap">{getOrdinalYear(member.year)} Year</td>
          ...
          <div className="md:hidden divide-y divide-gray-100 dark:divide-white/5">
            {alumniList.map((member) => (
              <div key={member.id} className="p-4 hover:bg-white/40 dark:hover:bg-white/5 transition-colors">
                ...
                <div className="mt-2.5 flex items-center gap-2 flex-wrap pl-13">
                  <span className="px-2.5 py-0.5 bg-white/60 dark:bg-dark-surface border border-gray-200/50 dark:border-white/10 rounded-md text-[11px] font-bold text-gray-700 dark:text-gray-300 shadow-sm">
                    {member.role}
                  </span>
                </div>
              </div>
        ...
        <div className="bg-white/60 dark:bg-dark-card/40 backdrop-blur-3xl rounded-3xl shadow-premium dark:shadow-premium-dark border border-white/60 dark:border-white/10 overflow-hidden transition-all duration-500 hover:shadow-premium-hover dark:hover:shadow-premium-dark-hover">
          ...
          <div className="md:hidden divide-y divide-gray-100 dark:divide-white/5">
            {formerMembersList.map((member) => (
              <div key={member.id} className="p-4 hover:bg-white/40 dark:hover:bg-white/5 transition-colors opacity-75 hover:opacity-100">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-100 dark:bg-dark-surface border border-gray-200/50 dark:border-white/10 flex items-center justify-center shrink-0">
                      {member.profileImage ? (
                ...
                <div className="mt-2.5 flex items-center gap-2 flex-wrap pl-13">
                  <span className="px-2.5 py-0.5 bg-white/60 dark:bg-dark-surface border border-gray-200/50 dark:border-white/10 rounded-md text-[11px] font-bold text-gray-600 dark:text-gray-400 shadow-sm">
                    {member.role}
                  </span>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                {formerMembersList.map((member) => (
                  <tr key={member.id} className="hover:bg-white/40 dark:hover:bg-white/5 transition-colors group opacity-60 hover:opacity-100">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">{getOrdinalYear(member.year)} Year</td>
                    <td className="px-4 py-3 relative text-right whitespace-nowrap">
                      <button 
                        type="button"
                        onClick={() => setActiveDropdownId(activeDropdownId === member.id ? null : member.id)}
