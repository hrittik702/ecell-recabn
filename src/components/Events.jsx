...
const Events = () => {
  ...
  return (
    <section ref={containerRef} id="events" className="py-24 relative z-10 bg-white dark:bg-transparent">
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="events-header flex flex-col md:flex-row md:items-end justify-between mb-16">
          <div>
            ...
            <h2 className="text-3xl md:text-5xl font-display font-bold text-gray-900 dark:text-white tracking-tight">National Achievements</h2>
          </div>
            ...
            return (
              ...
              >
                ...
                {event.image ? (
                  ...
                  <div className="absolute inset-0 z-0 bg-gradient-to-br from-indigo-50 dark:from-indigo-500/5 to-white dark:to-transparent opacity-50" />
                )}

                {/* Content Overlay */}
                <div className={`relative z-20 h-full flex flex-col justify-between p-8 md:p-10 ${event.image ? 'text-white md:w-3/4 lg:w-2/3' : 'text-gray-900 dark:text-white'}`}>
                  
                  {/* Top: Date & Arrow */}
                  <div className="flex justify-between items-start w-full">
                    <span className={`text-[10px] font-mono font-bold tracking-widest uppercase px-3 py-1.5 rounded-full border ${
                      event.image ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-gray-100 dark:bg-white/10 border-white/50 dark:border-white/10 text-gray-600 dark:text-gray-300'
                    }`}>
                      {event.date}
                    </span>
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border transition-transform duration-300 group-hover:scale-110 ${
                      event.image ? 'bg-white/10 border-white/20 text-white backdrop-blur-md' : 'bg-gray-100 dark:bg-white/10 border-white/50 dark:border-white/10 text-gray-900 dark:text-white'
                    }`}>
                  ...
                  <div className="mt-auto pt-8">
                    <span className={`text-[10px] font-mono font-bold tracking-widest uppercase mb-3 inline-block ${
                      event.image ? 'text-indigo-400' : 'text-indigo-600 dark:text-indigo-400'
                    }`}>
                      {event.tag}
                    </span>
                    <h3 className={`${isLarge ? 'text-3xl md:text-[44px]' : 'text-2xl md:text-3xl'} font-display font-extrabold mb-4 leading-tight ${event.image ? 'text-white drop-shadow-md' : 'text-gray-900 dark:text-white'}`}>
                      {event.title}
                    </h3>
                    <div className={event.image ? 'backdrop-blur-[2px] rounded-lg -mx-2 px-2' : ''}>
                      <p className={`text-base md:text-lg font-sans font-medium leading-relaxed max-w-2xl text-balance line-clamp-3 ${
                        event.image ? 'text-gray-200' : 'text-gray-600 dark:text-gray-300'
                      }`}>
                        {event.description}
                      </p>
                    </div>
                    ...
                    {event.link && (
                      <div className={`mt-6 inline-flex items-center text-sm font-bold tracking-wide transition-colors group/cta ${
                        event.image ? 'text-white hover:text-indigo-300' : 'text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300'
                      }`}>
                        ...
                        <div className={`absolute bottom-0 left-0 h-px transition-all duration-300 w-0 group-hover/cta:w-full ${
                          event.image ? 'bg-indigo-300' : 'bg-indigo-800 dark:bg-indigo-400'
                        }`} />
                      </div>
                    )}
                  </div>
